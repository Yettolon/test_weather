...
def post_list():
    ...
    try:
        ...
        if date>(datetime.now()+timedelta(minutes=1)) or date<(datetime.now()-timedelta(days=2)):
            return bad_request('date invalid')
        if -90.00 < float(request_data['lat']) < 90.00 and -180.00 < float(request_data['lng']) < 180.00:
            if request_data['temperature']:
                #writing a new record
                weather = WeathModel(latitude=float(request_data['lat']),
